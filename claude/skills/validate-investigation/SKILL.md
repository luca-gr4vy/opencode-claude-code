...
description: Validate investigation document against Jira and codebase
disable-model-invocation: true
context: fork
agent: deep
allowed-tools: Read, Edit, Write, Grep, Glob, Bash
argument-hint: [ticket-or-id] [optional-extra-context]
---

Validate the investigation document for feature `$0`. $1
...
Feature ticket:
!`acli jira workitem view $0 --fields summary,description,status,issuetype 2>/dev/null | head -c 4000 || echo "No Jira ticket for $0 - validate against the document's stated requirements."`
...
1. Read `INVESTIGATION-$0.md` and the Jira context above.
...
5. Edit `INVESTIGATION-$0.md` directly:
   ...
   - Suggest a `poc/$0` branch if not already mentioned
